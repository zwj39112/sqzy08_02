...
/**
 * @author ：ckw
 * @date ：Created in 2020/7/15 16:32
...
public class MyGeneratorNew {
    public static void main(String[] args) {
        ...
        sourceConfig.setUrl("jdbc:mysql://localhost:3306/tow_project?useUnicode=true&characterEncoding=utf8&serverTimezone=Asia/Shanghai");
        mpg.setDataSource(sourceConfig);
        // 策略配置
        StrategyConfig strategyConfig = new StrategyConfig();
        // .setCapitalMode(true)// 全局大写命名
        strategyConfig.setTablePrefix(new String[]{"tbl_"});// 此处可以修改为您的表前缀
        strategyConfig.setNaming(NamingStrategy.underline_to_camel);// 表名生成策略user_id  =>  userId
        mpg.setStrategy(strategyConfig);
        // 包配置
        PackageConfig packageConfig = new PackageConfig();
        packageConfig.setParent("com.aaa.ckw");// 自定义包路径
        ...
        injectionConfig.setFileOutConfigList(Collections.<FileOutConfig>singletonList(new FileOutConfig("/templates/mapper.xml.vm") {
            ...
            public String outputFile(TableInfo tableInfo) {
                return globalConfig.getOutputDir() + tableInfo.getEntityName() + "Dao.mapper";
            }
        }));
